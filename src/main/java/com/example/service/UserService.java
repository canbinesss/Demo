...
public class UserService extends ServiceImpl<UserMapper, User> {
    ...
    public User login(User user) {
        User one = getOne(Wrappers.<User>lambdaQuery().eq(User::getPassword, user.getPassword()).eq(User::getUsername, user.getUsername()).or().eq(User::getPhone, user.getUsername()));
        if (one == null) {
    ...
    public List<User> getUserIdNameList() {
        return userMapper.getUserIdNameList();
    }
