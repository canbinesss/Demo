...
public class FileController {
    ...
    public Result<String> getUploadUrl(MultipartFile file) {
        String filePath = System.getProperty("user.dir") + "//src/main/resources/static/file/";
        String flag = System.currentTimeMillis() + "";
        String fileName = file.getOriginalFilename();
        File file1 = null;
        try {
            file1 = FileUtil.writeBytes(file.getBytes(), filePath + flag + "-" + fileName);
            System.out.println(fileName + "--上传成功");
            Thread.sleep(1L);
        } catch (Exception e) {
            System.err.println(fileName + "--文件上传失败");
        }
        return Result.success("/files/"+flag + "-" + fileName);
    }
